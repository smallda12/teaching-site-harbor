...
(function () {
  ...
  const 相簿 = (function () {
    ...
    function 開() {
      return new Promise((res) => {
        if (db) return res(db);
        const r = indexedDB.open("認識港口照片", 1);
  ...
  const 點名 = (function () {
    ...
    async function 匯出() {
      ...
      const 資料 = { 格式: "認識港口點名名單", 版本: 1, 匯出時間: new Date().toISOString(), 學生: [] };
      for (const s of 名單) {
    ...
    async function 作答(選, 鈕) {
      ...
      if (對) {
        ...
        /* 總評量答錯不重答、也不說「再想一想」，只把正確答案標出來讓學生看一眼，
           然後直接進下一題（這裡是一次做完 18 題的總評量，不是練習） */
